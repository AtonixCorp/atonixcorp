...
def _mock_monthly_usage(year: int, month: int) -> list[dict]:
    ...
    from .billing_models import UNIT_PRICES
    seed = year * 12 + month
    items = []
...
def add_payment_method(owner, data: dict):
    ...
    from .billing_models import PaymentMethod
...
def get_billing_overview(owner) -> dict:
    from .billing_models import BillingAccount, Invoice, CreditNote
    acct = get_or_create_account(owner)
...
def _account_dict(acct) -> dict:
    from .billing_models import PLAN_PRICES, PLAN_FEATURES
    return {
        'id':           acct.id,
        'plan':         acct.plan,
...
def get_or_generate_invoice(owner, year: int, month: int):
    ...
    """
    from .billing_models import Invoice, InvoiceLineItem
...
def change_plan(owner, new_plan: str) -> dict:
    from .billing_models import BillingAccount, PLAN_PRICES, PlanTier
...
def get_credits(owner) -> list:
    from .billing_models import CreditNote
...
def apply_credit(owner, invoice_id: int, amount: Decimal) -> dict:
    from .billing_models import Invoice, BillingAccount
