                ...
                <TableRow key={p.id} sx={{ '& td': { borderColor: t.border } }}>
                  ...
                  <TableCell sx={{ py: 0.75 }}>
                    {p.scopes.split(',').map(sc => (
                      <Chip key={sc} label={sc} size="small" sx={{ mr: 0.4, height: 16, fontSize: '.62rem', bgcolor: 'rgba(21,61,117,.1)', color: S.info, '& .MuiChip-label': { px: 0.5 } }} />
                    ))}
                  </TableCell>
          ...
          {workspaces.map(ws => (
            <Chip
              ...
              sx={{
                ...
                bgcolor: ws.id === activeWsId ? 'rgba(21,61,117,.12)' : t.surface,
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          ...
          <Button
            ...
            sx={{ bgcolor: dashboardTokens.colors.brandPrimary, color: '#FFFFFF', fontWeight: 700,
              ...
              '&:hover': { bgcolor: dashboardTokens.colors.brandPrimaryHover, boxShadow: 'none' },
              '&.Mui-disabled': { bgcolor: 'rgba(21,61,117,.25)', color: 'rgba(255,255,255,.4)' } }}
          >
            {creating
